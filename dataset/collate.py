...
def resize_collate(max_area):
    def _resize_collate(max_allowed_area, batch):
        """
        ...
        if max_area > max_allowed_area:
            resize_factor = np.sqrt(max_allowed_area / max_area)
            max_shape = np.floor(max_shape * resize_factor)

            assert max_shape[0] * max_shape[1] <= max_allowed_area
    ...
    return partial(_resize_collate, max_area)
